...
// ------------------------------------------------------------
static const std::string CONN_INFO =
"host=localhost "
"port=5432 "
"dbname=mydb "
"user=root "
...
std::wstring action_handler(const std::wstring& data) {
    ...
    try {
        db = std::make_unique<DB>(CONN_INFO);
    ...
    try {    
        ...
        if (action == "todo_delete") {
            ...
            std::string sql = "DELETE FROM todos WHERE id = " + id_str + " RETURNING id";

            PGResult r(PQexec(db->get(), sql.c_str()));
            if (!r.ok())        return L"";
            if (r.rows() == 0)  return L"";
